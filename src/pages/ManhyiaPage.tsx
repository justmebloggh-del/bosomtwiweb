...
import { Crown, ArrowRight, PenSquare } from 'lucide-react';
...
const CATEGORY = 'Manhyia';

export default function ManhyiaPage({ articles, onArticleClick, onHomeClick, loading, user, onArticlePublished }: Props) {
  ...
  return (
    <div className="bg-news-bg min-h-screen text-news-text">
      ...
      {/* Header */}
      <header className="relative overflow-hidden border-b border-black/30 py-16 md:py-24 bg-gradient-to-br from-[#2D1A00] via-[#1A0F00] to-black">
        <motion.div animate={{ scale: [1, 1.4, 1], opacity: [0.25, 0.5, 0.25] }} transition={{ duration: 7, repeat: Infinity }}
          className="absolute -top-24 -left-24 w-96 h-96 rounded-full blur-[120px] bg-ashanti-gold" />
        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.3, 0.1] }} transition={{ duration: 9, repeat: Infinity, delay: 3 }}
          className="absolute -bottom-24 right-0 w-80 h-80 rounded-full blur-[100px] bg-yellow-600" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-end justify-between gap-6">
          <div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 mb-3">
              <Crown size={18} className="text-ashanti-gold" />
              <span className="text-ashanti-gold text-[11px] uppercase font-black tracking-[0.5em]">Royal Section</span>
            </motion.div>
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
              ...
              Royal palace, the Golden Stool, traditional leadership, and the enduring heritage of the Asante Kingdom.
            </motion.p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {filtered.length === 0 ? (
          <div className="text-center py-40 border-2 border-dashed border-brand-secondary/10 rounded-[3rem]">
            <Crown size={40} className="mx-auto mb-4 text-news-text/10" />
            <p className="text-news-text/40 font-heading italic text-2xl uppercase tracking-widest">No Manhyia stories yet</p>
            {user && (
              <button onClick={() => setShowPublish(true)} className="mt-6 px-8 py-3 bg-ashanti-gold text-black rounded-xl font-bold uppercase tracking-widest hover:scale-105 transition-all text-sm flex items-center gap-2 mx-auto">
            ...
            {rest.length > 0 && (
              <>
                <div className="flex items-center gap-4 mb-8">
                  <h2 className="text-[10px] font-black uppercase tracking-widest text-news-text/40">More from Manhyia</h2>
      ...
      <section className="bg-brand-surface py-14 border-y border-brand-secondary/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="font-heading text-2xl font-bold text-news-text uppercase">Explore More Bosomtwi Web</h2>

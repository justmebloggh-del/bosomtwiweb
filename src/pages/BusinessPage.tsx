...
import { TrendingUp, ArrowRight, PenSquare } from 'lucide-react';
...
export default function BusinessPage({ articles, onArticleClick, onHomeClick, loading, user, onArticlePublished }: Props) {
  ...
  return (
    <div className="bg-news-bg min-h-screen text-news-text">
      ...
      <header className="relative overflow-hidden border-b border-black/30 py-16 md:py-24 bg-gradient-to-br from-[#002B1A] via-[#001510] to-black">
        <motion.div animate={{ scale: [1, 1.4, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 7, repeat: Infinity }}
          className="absolute -top-24 -left-24 w-96 h-96 rounded-full blur-[120px] bg-green-700" />
        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.25, 0.1] }} transition={{ duration: 9, repeat: Infinity, delay: 3 }}
          className="absolute -bottom-24 right-0 w-80 h-80 rounded-full blur-[100px] bg-emerald-900" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-end justify-between gap-6">
          <div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 mb-3">
              ...
              <span className="text-green-400 text-[11px] uppercase font-black tracking-[0.5em]">Economy & Markets</span>
            </motion.div>
            <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
              className="font-heading text-6xl md:text-9xl font-black text-white uppercase leading-none tracking-tighter">
              Business
            </motion.h1>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
              className="mt-3 text-white/50 text-sm max-w-xl">
              Markets, enterprise, investment, trade, and economic development from the Ashanti Region and across Africa.
            </motion.p>
          ...
          {user && (
            <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="shrink-0 flex items-center gap-2 bg-green-600 text-white px-5 py-3 rounded-xl font-black text-[11px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {filtered.length === 0 ? (
          <div className="text-center py-40 border-2 border-dashed border-brand-secondary/10 rounded-[3rem]">
            <TrendingUp size={40} className="mx-auto mb-4 text-news-text/10" />
            <p className="text-news-text/40 font-heading italic text-2xl uppercase tracking-widest">No Business stories yet</p>
            {user && (
              <button onClick={() => setShowPublish(true)} className="mt-6 px-8 py-3 bg-green-600 text-white rounded-xl font-bold uppercase tracking-widest hover:scale-105 transition-all text-sm flex items-center gap-2 mx-auto">
            ...
            {rest.length > 0 && (
              <>
                <div className="flex items-center gap-4 mb-8">
                  <h2 className="text-[10px] font-black uppercase tracking-widest text-news-text/40">More from Business</h2>

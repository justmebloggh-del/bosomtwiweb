...
import { Cpu, ArrowRight, PenSquare } from 'lucide-react';
...
export default function TechnologyPage({ articles, onArticleClick, onHomeClick, loading, user, onArticlePublished }: Props) {
  ...
  return (
    <div className="bg-news-bg min-h-screen text-news-text">
      ...
      <header className="relative overflow-hidden border-b border-black/30 py-16 md:py-24 bg-gradient-to-br from-[#001D2E] via-[#000D15] to-black">
        <motion.div animate={{ scale: [1, 1.4, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 7, repeat: Infinity }}
          className="absolute -top-24 -left-24 w-96 h-96 rounded-full blur-[120px] bg-cyan-700" />
        <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.25, 0.1] }} transition={{ duration: 9, repeat: Infinity, delay: 3 }}
          className="absolute -bottom-24 right-0 w-80 h-80 rounded-full blur-[100px] bg-teal-900" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-end justify-between gap-6">
          <div>
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 mb-3">
              ...
              <span className="text-cyan-400 text-[11px] uppercase font-black tracking-[0.5em]">Innovation & Science</span>
            </motion.div>
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
              ...
              Innovation, digital transformation, artificial intelligence, startups, and science shaping Africa's future.
            </motion.p>
          ...
          {user && (
            <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
              ...
              className="shrink-0 flex items-center gap-2 bg-cyan-600 text-white px-5 py-3 rounded-xl font-black text-[11px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {filtered.length === 0 ? (
          <div className="text-center py-40 border-2 border-dashed border-brand-secondary/10 rounded-[3rem]">
            <Cpu size={40} className="mx-auto mb-4 text-news-text/10" />
            <p className="text-news-text/40 font-heading italic text-2xl uppercase tracking-widest">No Technology stories yet</p>
            {user && (
              <button onClick={() => setShowPublish(true)} className="mt-6 px-8 py-3 bg-cyan-600 text-white rounded-xl font-bold uppercase tracking-widest hover:scale-105 transition-all text-sm flex items-center gap-2 mx-auto">
            ...
            {rest.length > 0 && (
              <>
                <div className="flex items-center gap-4 mb-8">
                  <h2 className="text-[10px] font-black uppercase tracking-widest text-news-text/40">More from Technology</h2>
